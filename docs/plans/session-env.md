...
- **내용 해시를 비교해 갱신**한다. 해시가 같으면 아무것도 하지 않고, 다르면 다시
  복사한다(자격증명은 도는 값이라 "1회만"이면 곤란하다). 마커는 컨테이너 안
  `$XDG_CACHE_HOME/cld/files/`에 둔다.
- 배치 시점은 dotfiles 다음, 세션 생성 전.
- 다만 **타이머가 아니라 프로비저닝 경로**에서 돈다. `ensure_`는 컨테이너가 ready로
  안정되면 조기 반환하므로, 갱신은 컨테이너 기동·데몬 재시작·재프로비저닝 때
  일어난다. 세션이 떠 있는 동안 도는 자격증명은 다음 세션에 반영된다(→ [추후](#추후)).
...
- [x] 5. `files` 배치 — 해시 비교 갱신 (`internal/daemon/files.go`).
...
- `files`의 주기적 재배치. 지금은 프로비저닝 때만 해시를 비교하므로, 세션이 살아
  있는 동안 호스트에서 갱신된 자격증명은 컨테이너에 반영되지 않는다.
- `pre_attach` 훅.
