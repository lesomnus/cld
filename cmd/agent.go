...
// effort: any failure just means that piece is unavailable. Claude Code's own
// user-level config is not staged here: the daemon reads it directly out of
// cld's user-default dir (see config.Config.UserDefaultDir), which is already
// the persistent, user-managed source — no host file is ever touched.
